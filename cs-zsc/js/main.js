;(function () {
	...
	var burgerMenu = function() {
		...
		$('body').on('click', '.js-cs-nav-toggle', function(event){
	...
	var windowScroll = function() {
		...
		$(window).scroll(function(event){
			...
			if ( scrlTop > 500 && scrlTop <= 2000 ) {
				header.addClass('navbar-fixed-top cs-animated slideInDown');
			} else if ( scrlTop <= 500) {
				if ( header.hasClass('navbar-fixed-top') ) {
					header.addClass('navbar-fixed-top cs-animated slideOutUp');
					setTimeout(function(){
						header.removeClass('navbar-fixed-top cs-animated slideInDown slideOutUp');
					}, 100 );
				}
			} 
	...
	var servicesAnimate = function() {
		...
		if ( $('#cs-our-services').length > 0 ) {	
			$('#cs-our-services .to-animate').each(function( k ) {
	...
	var servicesWayPoint = function() {
		...
		if ( $('#cs-our-services').length > 0 ) {
			$('#cs-our-services').waypoint( function( direction ) {
	...
	var featuresAnimate = function() {
		...
		if ( $('#cs-features').length > 0 ) {	
			$('#cs-features .to-animate').each(function( k ) {
	...
	var featuresWayPoint = function() {
		...
		if ( $('#cs-features').length > 0 ) {
			$('#cs-features').waypoint( function( direction ) {
	...
	var testimonialsAnimate = function() {
		...
		if ( $('#cs-testimonials').length > 0 ) {	
			$('#cs-testimonials .to-animate').each(function( k ) {
	...
	var testimonialsWayPoint = function() {
		...
		if ( $('#cs-testimonials').length > 0 ) {
			$('#cs-testimonials').waypoint( function( direction ) {
	...
	var pricingAnimate = function() {
		...
		if ( $('#cs-pricing').length > 0 ) {	
			$('#cs-pricing .to-animate').each(function( k ) {
	...
	var pricingWayPoint = function() {
		...
		if ( $('#cs-pricing').length > 0 ) {
			$('#cs-pricing').waypoint( function( direction ) {
	...
	var pressAnimate = function() {
		...
		if ( $('#cs-press').length > 0 ) {	
			$('#cs-press .to-animate').each(function( k ) {
	...
	var pressWayPoint = function() {
		...
		if ( $('#cs-press').length > 0 ) {
			$('#cs-press').waypoint( function( direction ) {
